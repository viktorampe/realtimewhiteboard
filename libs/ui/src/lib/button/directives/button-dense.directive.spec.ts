...
import { DenseDirective } from './button-dense.directive';

@Component({
  // tslint:disable-next-line:component-selector
  selector: 'test-container',
  template: `
    <campus-button button-dense>tekst</campus-button>
  `
})
...
describe('DenseDirective', () => {
  let directive: DenseDirective;
  ...
  beforeEach(() => {
    ...
    directive = componentDE.injector.get(DenseDirective);
  });
  ...
  it('should apply the correct class', () => {
    expect(componentDE.nativeElement.classList).toContain('ui-button--dense');
  });
});
