...
describe('ResultEffects', () => {
  ...
  describe('loadResult$', () => {
    const unforcedLoadAction = new LoadResults({ userId: 1 });
    const forcedLoadAction = new LoadResults({ force: true, userId: 1 });
    const filledLoadedAction = new ResultsLoaded({ results: [] });
    const loadErrorAction = new ResultsLoadError(new Error('failed'));
    describe('with initialState', () => {
      beforeAll(() => {
        usedState = ResultReducer.initialState;
      });
      beforeEach(() => {
    ...
    describe('with loaded state', () => {
      beforeAll(() => {
        usedState = { ...ResultReducer.initialState, loaded: true };
      });
      beforeEach(() => {
        mockServiceMethodReturnValue('getAllForUser', []);
      });
